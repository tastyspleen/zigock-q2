...
static void strip_bot_tag_from_client_name (const char *name, char *dest, int destlen)
{
...
void ClientUserinfoChanged(edict_t *ent, char *userinfo)
{
	...
	s = Info_ValueForKey(userinfo, "name");
	{
		...
		if (! ENT_IS_BOT(ent)) {
			...
			strip_bot_tag_from_client_name(name, name_buf, sizeof(name_buf));
			if (strlen(name_buf) < 1) {
				strcpy(name_buf, "BOZO");  // their name was nothing but bot tags?
			}
			name = name_buf;
		}
		...
		if (targetindex > 0) {
			...
			if (!TraceX(ent, other->s.origin)) {
				...
				while (1) {
					if (i + 1 >= CurrentIndex) {
						j = Route[i + 1].state;
						if (j == GRS_ONTRAIN) if (Route[i + 1].ent->trainteam) {
								break;
							}
...
//gi.bprintf(PRINT_HIGH,"2\n");
			...
			if (ent->groundentity) {
				if (!Q_stricmp(ent->groundentity->classname, "func_train")
				    ...
				    || !Q_stricmp(ent->groundentity->classname, "func_door")) {
					i = false;
				}
			}
